...
import { FlowState, FlowResult } from "./types.js";
...
export interface InstanceRecord {
  instanceId: string;
  flowName: string;
  status: "running" | "completed" | "paused" | "waiting" | "failed" | "cancelled";
  state: FlowState;
...
export class StateStore {
  ...
  constructor(stateDir?: string) {
    this.dir = stateDir ?? path.join(process.env.HOME ?? ".", ".openclaw", "flow-state");
    fs.mkdirSync(this.dir, { recursive: true });
  }

  create(instanceId: string, flowName: string, initialState: FlowState): InstanceRecord {
  ...
  update(instanceId: string, patch: Partial<InstanceRecord>): InstanceRecord {
  ...
  // Mark a node's output as complete — won't re-run on resume
  ...
  // Check if a node was already completed in a prior run
  getMemoized(instanceId: string, nodeName: string): { found: boolean; output: unknown } {
    const record = this.get(instanceId);
    if (!record) return { found: false, output: undefined };
    const has = Object.prototype.hasOwnProperty.call(record.completedNodes, nodeName);
  ...
  list(status?: string): InstanceRecord[] {
    ...
    const records = files
      .map((f) => {
        try { return JSON.parse(fs.readFileSync(path.join(this.dir, f), "utf8")) as InstanceRecord; }
        catch { return null; }
      })
  ...
  private write(record: InstanceRecord): void {
    fs.writeFileSync(this.filePath(record.instanceId), JSON.stringify(record, null, 2));
  }
}
